(function () {
  ...
  style.textContent = [
    ...
    ".aaa-fab .aaa-chat-launch:focus,.aaa-fab .aaa-chat-launch:focus-visible{outline:none !important;box-shadow:0 0 0 3px " + CRIMSON + " !important}",
    ...
    ".aaa-chat-panel{position:fixed;bottom:164px;right:20px;z-index:2147483000;width:380px;max-width:calc(100vw - 32px);height:560px;max-height:calc(100vh - 198px);background:#0a1b2c;border:1.5px solid " + CRIMSON + ";box-shadow:0 20px 50px rgba(3,7,11,0.6);border-radius:16px;display:none;flex-direction:column;overflow:hidden;font-family:'Roboto',system-ui,-apple-system,'Segoe UI',sans-serif}",
    ...
    ".aaa-chat-header{background:" + NAVY + ";color:#fff;padding:16px 18px;display:flex;align-items:center;gap:12px;border-bottom:3px solid " + CRIMSON + "}",
    ...
    ".aaa-chat-control-btn:hover{opacity:1;background-color:rgba(255,255,255,0.15);color:" + CRIMSON + "}",
    ".aaa-chat-control-btn:focus,.aaa-chat-control-btn:focus-visible,.aaa-chat-control-btn:active{outline:none !important;box-shadow:none !important}",
    ".aaa-chat-close:focus,.aaa-chat-close:focus-visible,.aaa-chat-close:active{outline:none !important;box-shadow:none !important}",
    ...
    ".aaa-chat-log{flex:1;overflow-y:auto;padding:18px;background:#050e16;display:flex;flex-direction:column;gap:12px}",
    ".aaa-chat-log::-webkit-scrollbar{width:6px}",
    ".aaa-chat-log::-webkit-scrollbar-track{background:transparent}",
    ".aaa-chat-log::-webkit-scrollbar-thumb{background:#1B2A4A;border-radius:3px}",
    ...
    ".aaa-msg-avatar{width:30px;height:30px;border-radius:9999px;object-fit:cover;flex:0 0 30px;border:1.5px solid " + CRIMSON + ";box-shadow:0 2px 6px rgba(13,34,55,.16)}",
    ...
    ".aaa-msg.aaa-bot{background:#fff;color:#0d2237;border:1px solid #cbd5e1;border-bottom-left-radius:4px}",
    ...
    ".aaa-chat-prompts{padding:10px 12px 9px;background:#fff;border-top:1px solid #cbd5e1}",
    ".aaa-chat-prompts-label{margin:0 0 7px;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#64748b}",
    ...
    ".aaa-chat-prompt{flex:0 0 auto;max-width:230px;border:1.5px solid " + CRIMSON + ";border-radius:9999px;background:#0d2237;color:#fff;padding:8px 11px;font:700 12px/1.2 'Roboto',system-ui,-apple-system,'Segoe UI',sans-serif;cursor:pointer;white-space:nowrap;transition:transform .15s ease,border-color .15s ease,background .15s ease,color .15s ease}",
    ".aaa-chat-prompt:hover:not(:disabled){transform:translateY(-1px);border-color:" + CRIMSON + ";background:" + CRIMSON + ";color:#fff}",
    ".aaa-chat-prompt:focus-visible{outline:3px solid " + CRIMSON + ";outline-offset:1px}",
    ".aaa-chat-prompt:disabled{opacity:.48;cursor:not-allowed}",
    ".aaa-chat-emoji-bar{display:flex;gap:6px;padding:8px 12px;background:#f1f5f9;border-top:1px solid #cbd5e1;align-items:center;overflow-x:auto;scrollbar-width:none}",
    ".aaa-chat-emoji-bar[hidden]{display:none}",
    ".aaa-chat-emoji-bar::-webkit-scrollbar{display:none}",
    ".aaa-chat-emoji{flex:0 0 auto;width:34px;height:34px;border:1.5px solid " + CRIMSON + ";border-radius:10px;background:#0d2237;font-size:19px;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s ease,border-color .15s ease,transform .15s ease}",
    ".aaa-chat-emoji:hover{background:" + CRIMSON + ";border-color:" + CRIMSON + ";transform:translateY(-2px) rotate(-3deg)}",
    ".aaa-chat-emoji:focus-visible{outline:3px solid " + CRIMSON + ";outline-offset:1px}",
    ".aaa-chat-form{border-top:1px solid #cbd5e1;padding:12px;display:flex;gap:8px;align-items:flex-end;background:#fff}",
    ".aaa-chat-form textarea{flex:1;resize:none;border:1px solid #94a3b8;border-radius:12px;padding:10px 12px;font-family:inherit;font-size:14px;line-height:1.4;max-height:120px;color:#0d2237;transition:border-color .15s ease,box-shadow .15s ease}",
    ".aaa-chat-form textarea:focus{outline:none;border-color:" + CRIMSON + ";box-shadow:0 0 0 3px rgba(166,31,46,0.15)}",
    ".aaa-chat-emoji-trigger{flex-shrink:0;width:42px;height:42px;border-radius:12px;border:1.5px solid " + CRIMSON + ";background:#0d2237;color:#fff;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s ease,color .15s ease,border-color .15s ease}",
    ".aaa-chat-emoji-trigger:hover,.aaa-chat-emoji-trigger[aria-expanded='true']{background:" + CRIMSON + ";color:#fff;border-color:" + CRIMSON + "}",
    ".aaa-chat-emoji-trigger:focus-visible{outline:3px solid " + CRIMSON + ";outline-offset:1px}",
    ...
    ".dark .aaa-chat-header p{color:#cbd5e1;opacity:1}",
    ".dark .aaa-chat-log{background:#050e16}",
    ".dark .aaa-msg.aaa-bot{background:#0a1b2c;color:#fff;border-color:#1B2A4A}",
    ".dark .aaa-msg.aaa-bot a{color:#fb923c}",
    ".dark .aaa-msg.aaa-user{background:" + CRIMSON + ";color:#fff}",
    ".dark .aaa-msg-avatar{border-color:#1B2A4A}",
    ".dark .aaa-typing span{background:" + CRIMSON + "}",
    ".dark .aaa-chat-prompts{background:#0a1b2c;border-top-color:#1B2A4A}",
    ".dark .aaa-chat-prompts-label{color:#94a3b8}",
    ".dark .aaa-chat-prompt{background:#0a1b2c;color:#fff;border-color:" + CRIMSON + "}",
    ".dark .aaa-chat-prompt:hover:not(:disabled){background:" + CRIMSON + ";border-color:" + CRIMSON + ";color:#fff}",
    ".dark .aaa-chat-emoji-bar{background:#0a1b2c;border-top-color:#1B2A4A}",
    ".dark .aaa-chat-emoji{background:#0a1b2c;border-color:" + CRIMSON + "}",
    ".dark .aaa-chat-emoji:hover{background:" + CRIMSON + ";border-color:" + CRIMSON + "}",
    ".dark .aaa-chat-form{background:#0a1b2c;border-top-color:#1B2A4A}",
    ".dark .aaa-chat-form textarea{background:#050e16;color:#fff;border-color:#1B2A4A}",
    ".dark .aaa-chat-form textarea:focus{border-color:" + CRIMSON + ";box-shadow:0 0 0 3px rgba(142,31,38,0.25)}",
    ...
    "@media(max-width:767px){.aaa-fab{right:max(16px,env(safe-area-inset-right));bottom:calc(80px + env(safe-area-inset-bottom,0px));flex-direction:column;gap:12px;align-items:flex-end}.aaa-fab .aaa-fab-btn{min-width:0;width:54px;height:54px;padding:0;border-radius:50%;gap:0;flex:0 0 54px}.aaa-fab .aaa-fab-btn i{font-size:21px;line-height:1}.aaa-fab .aaa-fab-label{display:none!important}.aaa-fab-btn.aaa-chat-launch{display:flex!important}.aaa-fab-btn.aaa-call{display:none!important}.aaa-chat-bubble{display:none!important}.aaa-chat-panel{position:fixed;top:0;left:0;right:0;bottom:0;width:100%!important;max-width:100%!important;height:100dvh!important;max-height:100dvh!important;border-radius:0!important;border:none!important;z-index:2147483005}.aaa-chat-header{padding:calc(16px + env(safe-area-inset-top,0px)) 18px 16px!important}.aaa-chat-disclaimer{padding:0 12px calc(10px + env(safe-area-inset-bottom,0px))!important}}",
