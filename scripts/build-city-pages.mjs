...
/*
 * Prune & redirect: only cities flagged `landing: true` in
 * public/data/service-areas.json get a generated page. Stale files from
 * earlier builds (cities that lost their flag, or old slugs) are deleted and
 * recorded in public/_redirects so their URLs 301 to /service-areas.
 */
...
const landingBySlug = Object.fromEntries(LANDINGS.map((c) => [c.slug, c]));

mkdirSync(OUT_DIR, { recursive: true });

// Stale pruned pages are removed from disk and folded into the redirect table.
// The managed block persists across runs: this run's deletions merge with the
// slugs already recorded there, and a slug that regains a landing page drops
// back out so its force redirect never shadows the regenerated file.
//
// The managed block is (re)inserted BEFORE the "/handyman/*" splat rewrite.
// Netlify processes rules top to bottom and serves the first match; the splat
// is a 200 rewrite with no fall-through when its destination is missing, so
// any pruned-slug rule placed after it would never fire and those URLs would
// 404 instead of redirecting.
const REDIRECT_HEADER_LINES = [
  '# Generated by scripts/build-city-pages.mjs -- do not edit.',
  '# Pruned city pages fold onto the main service-areas page.'
];
const SPLAT_LINE = '/handyman/* /handyman/:splat.html 200';
const redirectsPath = join(ROOT, 'public/_redirects');
const redirectLines = readFileSync(redirectsPath, 'utf8').split('\n');

// Lift the previous managed block out of the file, wherever it sits, and note
// the slugs it carried. Everything else is treated as hand-maintained base.
const markerIdx = redirectLines.findIndex((l) => l.startsWith(REDIRECT_HEADER_LINES[0]));
let existingSlugs = [];
if (markerIdx !== -1) {
  let end = markerIdx + 1;
  while (end < redirectLines.length && (redirectLines[end].startsWith('#') || redirectLines[end].startsWith('/handyman/'))) end += 1;
  existingSlugs = Array.from(
    redirectLines.slice(markerIdx, end).join('\n').matchAll(/\/handyman\/([a-z0-9-]+) \/service-areas 301!/g),
    (m) => m[1]
  );
  redirectLines.splice(markerIdx, end - markerIdx);
}
...
const prunedSlugs = Array.from(new Set([...existingSlugs, ...pruned]))
  .filter((slug) => !landingBySlug[slug])
  .sort();
if (prunedSlugs.length) {
  const splatIdx = redirectLines.findIndex((l) => l === SPLAT_LINE);
  if (splatIdx === -1) {
    throw new Error('public/_redirects is missing the /handyman/* splat rule; cannot place pruned-city redirects ahead of it.');
  }
  const blockLines = [
    ...REDIRECT_HEADER_LINES,
    ...prunedSlugs.map((slug) => `/handyman/${slug} /service-areas 301!`),
    ''
  ];
  redirectLines.splice(splatIdx, 0, ...blockLines);
}
...
// Cities with their own landing page, in data-file order. Used for the
// "nearby areas" chips and the all-cities index block on every page.
const ALL_LANDING_SLUGS = LANDINGS.map((c) => c.slug);

function nearbyLinks(city) {
  const nearby = (city.nearby || []).map((slug) => landingBySlug[slug]).filter(Boolean);
...
function citiesIndex(city) {
  const links = ALL_LANDING_SLUGS
    .filter((slug) => slug !== city.slug)
    .map((slug) => {
      const c = landingBySlug[slug];
      return `<a href="/handyman/${esc(slug)}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-red-50 hover:text-red-600 rounded-xl transition border border-gray-200 font-semibold text-gray-800"><i class="fas fa-map-marker-alt text-red-500" aria-hidden="true"></i> Handyman in ${esc(c.name)}</a>`;
    })
    .join('\n                    ');
  return `
            <!-- All primary service areas: crawlable internal index -->
            <div class="max-w-5xl mx-auto mt-12 sm:mt-16 border-t border-gray-200 pt-10">
                <h2 class="text-2xl sm:text-3xl font-bold text-blue-900 text-center mb-6">Our Primary Service Areas</h2>
                <p class="text-center text-gray-600 mb-6 max-w-2xl mx-auto">Dedicated handyman pages for the ${LANDINGS.length} Oakland County communities we serve most:</p>
                <div class="flex flex-wrap justify-center gap-3">
                    ${links}
                </div>
            </div>`;
...
function cityFaq(city) {
  ...
  const jobs = Array.isArray(city.jobs) ? city.jobs : [];
  const localQ = (housing || jobs.length)
    ? {
        q: `What handyman jobs are common in ${city.name} homes?`,
        a: [
          housing ? `${housing} That stock shapes the work we do here.` : '',
          jobs.length ? `Recent ${city.name} calls include ${jobs.slice(0, 2).join(', ')}, and ${jobs[2] || 'similar repairs'}.` : ''
        ].filter(Boolean).join(' ')
      }
    : null;
  const base = [
  ...
  return localQ ? [base[0], localQ, base[1], base[2]] : base;
}
...
function page(city) {
  ...
  const hint = city.metaHint ? ` ${city.metaHint}` : '';
  const desc = `Local handyman for ${city.name}, MI homes &mdash;${hint} $100 minimum, $70/hr after. Call ${PHONE_DISPLAY}.`;
        ...
        <div class="max-w-7xl mx-auto px-6 pt-10 sm:pt-14">
            ...
            <div class="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto items-start reveal-on-scroll">
                <div class="lg:col-span-2 prose prose-lg max-w-none text-gray-600">
                    ...
                    <p>${esc(city.blurb)}</p>
${housing ? `                    <p>${esc(housing)}</p>
` : ''}${city.closer ? `                    <p>${esc(city.closer)}</p>
` : ''}                    <p>${esc(city.bridge || `Whether it is a single nagging repair or a full seasonal to-do list, we bring the same craftsmanship, clean job sites, and clear communication to every ${city.name} home.`)}</p>
${landmark ? `                    <p class="text-gray-700"><i class="fas fa-map-pin text-red-600 mr-1.5" aria-hidden="true"></i>You'll find our vans ${esc(landmark)}.</p>
` : ''}                </div>
...
${jobs.length ? `            <!-- Recent ${esc(city.name)} work -->
            <div class="max-w-6xl mx-auto mt-12 sm:mt-16">
                <div class="text-center mb-8">
                    <div class="uppercase text-blue-600 font-semibold tracking-widest text-sm">Recent Jobs</div>
                    <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold mt-2 text-blue-900">The Kind of Work We Do in ${esc(city.name)}</h2>
                    <p class="mt-3 text-gray-600 max-w-2xl mx-auto">Real examples of the repairs and projects ${esc(city.name)} homeowners call us for:</p>
                </div>
                <div class="grid sm:grid-cols-3 gap-4">
${jobs.map((j) => `                    <div class="card-layered flex items-start gap-3.5 bg-white border border-slate-200/90 p-5 rounded-2xl shadow-sm hover:shadow-md hover:border-red-600/40 transition-all">
                        <span class="w-9 h-9 flex-shrink-0 bg-red-100/80 rounded-xl flex items-center justify-center text-red-600" aria-hidden="true"><i class="fas fa-screwdriver-wrench" aria-hidden="true"></i></span>
                        <span class="text-sm font-medium text-gray-700 self-center">${esc(j)}</span>
                    </div>`).join('\n')}
                </div>
            </div>
` : ''}
            ...
            <div class="grid gap-10 md:grid-cols-3 md:gap-8 text-center md:text-left">
                <div class="flex flex-col items-center md:items-start">
                    ...
                    <div class="mt-4 flex items-center gap-4">
                        ...
                        <a href="https://maps.app.goo.gl/uqK9xsJj4UX5tW4W7" target="_blank" rel="noopener noreferrer" aria-label="Find AAA Handyman Services LLC on Google" class="text-2xl text-[#4285F4] hover:opacity-80 transition"><svg width="1em" height="1em" viewBox="0 0 48 48" aria-hidden="true" focusable="false"><path fill="#4285F4" d="M45.12 24.5c0-1.56-.14-3.06-.4-4.5H24v8.64h7.11c4.16-3.83 6.56-9.47 6.56-16.64z"/><path fill="#34A853" d="M24 46c5.94 0 10.92-1.97 14.56-5.33l-7.11-5.7c-1.97 1.32-4.49 2.1-7.45 2.1-5.73 0-10.58-3.87-12.31-9.07H4.34v5.7C7.96 42.41 15.4 46 24 46z"/><path fill="#FBBC05" d="M11.69 28.18C11.25 26.86 11 25.45 11 24s.25-2.86.69-4.18v-5.7H4.34C2.85 17.09 2 20.45 2 24c0 3.55.85 6.91 2.34 9.88l7.35-5.7z"/><path fill="#EA4335" d="M24 9.75c3.23 0 6.13 1.11 8.41 3.29l6.31-6.31C34.91 3.18 29.93 1 24 1 15.4 1 7.96 4.59 4.34 11.12l7.35 5.7C13.42 13.62 18.27 9.75 24 9.75z"/></svg></a>
                    </div>
                </div>
...
pruned.length && console.log(`Pruned ${pruned.length} stale city page(s): ${pruned.join(', ')}`);
